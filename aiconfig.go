...
func NewAIConfigManager(configService *ConfigService) (*AIConfigManager, error) {
...
func (acm *AIConfigManager) SetAPIKey(provider AIProvider, apiKey string) error {
	acm.mu.Lock()
	defer acm.mu.Unlock()
...
func (acm *AIConfigManager) SetPreferredProvider(provider AIProvider) error {
	acm.mu.Lock()
	defer acm.mu.Unlock()
...
func (acm *AIConfigManager) IsProviderConfigured(provider AIProvider) bool {
	acm.mu.RLock()
	defer acm.mu.RUnlock()
...
func (acm *AIConfigManager) GetProviderStatus() map[AIProvider]ProviderStatus {
	acm.mu.RLock()
	defer acm.mu.RUnlock()

	status := make(map[AIProvider]ProviderStatus)
