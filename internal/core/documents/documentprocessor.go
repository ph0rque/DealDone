...
// DocumentType and DocumentInfo are defined in types.go
...
func NewDocumentProcessor(aiService *ai.AIService) *DocumentProcessor {
	return &DocumentProcessor{
		aiService:  aiService,
		ocrService: ocr.NewOCRService(""), // Will be configured later
		supportedExts: map[string]bool{
			".pdf":  true,
			".doc":  true,
			".docx": true,
...
func (dp *DocumentProcessor) SetOCRService(ocrService *ocr.OCRService) {
...
func (dp *DocumentProcessor) ProcessDocument(filePath string) (*DocumentInfo, error) {
	...
	if ruleBasedType != DocTypeGeneral {
		docInfo.Type = string(ruleBasedType)
	...
	if dp.aiService != nil {
		...
		if err == nil {
			docInfo.Type = string(aiResult.Type)
